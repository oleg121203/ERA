...
В этом проекте представлена только JavaScript версия игры "Змейка".
...
## Управление
- **Игрок:**
  - Стрелки или клавиши WASD для управления.

## Звуковые эффекты
- Все звуковые эффекты теперь в формате `.mp3`.
- Убедитесь, что файлы `eat.mp3` и `game_over.mp3` находятся в директории `snake_js/sounds/`.

## Настройки игры
В верхнем правом углу нажмите кнопку "Настройки" для открытия выдвижной панели. Вы можете изменить следующие настройки:
- **Скорость змейки:** Используйте ползунок для изменения скорости.
- **Цвет змейки:** Используйте цветовой пипет для выбора цвета.
- **Фоновое изображение:** Выберите из доступных вариантов в выпадающем списке.
- **Тема:** Выберите светлую или темную тему интерфейса.
- **Форма змеи:** Выберите форму змеи (классическая, круглая, треугольная, пользовательская).
- **Узор змеи:** Выберите узор змеи (сплошной, полосатый, точечный, градиент).
- **Размер сетки:** Используйте ползунок для изменения размера сетки.
- **Сложность игры:** Выберите уровень сложности (легкая, средняя, сложная).
